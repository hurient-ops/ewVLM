...
- 백엔드에 `onvif-zeep` 라이브러리를 실제 환경에 설치(requirements.txt 추가)하여 Mock이 아닌 실제 ONVIF 제어 모드가 동작하도록 구성을 완료했습니다.

## 8. PTZ GIS 맵 연동 (PtzHandoverConsole.tsx)
- 기존에 정적 이미지(Mock)로만 처리되어 있던 관제 맵 영역을 SVG 기반의 동적 다이내믹 렌더링(Dynamic Rendering) 뷰로 전면 교체했습니다.
- 이제 현재 추적 중인 카메라의 FOV(시야각) 렌더링, 위치 추적 애니메이션이 라이브로 표출되며, 사용자가 맵 상의 카메라를 클릭해 즉시 핸드오버를 트리거할 수 있도록 구축했습니다.

## 9. 자원 모니터링 대시보드 리얼 차트 (EdgeAiOrchestration.tsx)
- CPU 등 하드웨어 자원 상태를 보여주는 부분을 더 이상 `<div>` 블록으로 된 정적 더미가 아닌, 실제 [recharts](https://recharts.org/) 차트 라이브러리(`BarChart`)로 렌더링되게 변경했습니다.
- 백엔드 `getNvrStatus` 응답 데이터와 연계되어 노드를 선택할 때마다 동적으로 실시간 차트가 그려지도록 연동을 완료했습니다.
